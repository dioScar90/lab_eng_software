class Carro {
    ...
    constructor(marca, modelo, ano, cor, quilometragem, valorFipe) {
        this.#marca = marca;
        this.#modelo = modelo;
        this.#ano = +ano;
        this.#cor = cor;
...
function adicionarCarroSimples(values) {
    ...
    const details = document.querySelector("#details-" + modelo);
    const ol = details.lastElementChild;

    ol.insertAdjacentHTML("beforeend", `<li>${marca}</li>`);
...
function adicionarCarroDetalhado(values) {
    ...
    const carro = new Carro(marca, modelo, ano, cor, quilometragem, valorFipe);
...
function limparCarrosDetalhado() {
    const divDetalhes = document.querySelector("#div-detalhes");
...
function limparCarros(e) {
    if (e.target.id == "form-carro-simples") {
        limparCarrosSimples();
        return;
    }

    limparCarrosDetalhado();
}
...
document.querySelector("#form-carro-simples").addEventListener("reset", limparCarros);
document.querySelector("#form-carro-detalhado").addEventListener("submit", adicionarCarro);
document.querySelector("#form-carro-detalhado").addEventListener("reset", limparCarros);
